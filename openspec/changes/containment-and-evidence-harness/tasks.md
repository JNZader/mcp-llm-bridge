...
## Canonical units (61 rows; 4 checked, 57 unchecked)
...
- [ ] 2.3 ERR-HTTP-FOUNDATION [DAG§2](design/scenarios-dag.md#2-concrete-work-unit-ledger)
...
The checklist remains exactly 61 rows: 4 checked and 57 unchecked. `2.1 AUTH-RUNTIME` is admitted and checked after its scoped Node 22 focused-test, typecheck, and hygiene gate. The exact root command remains mandatory REQ-TEST-01 evidence owned later by CI-ROOT, not a per-unit AUTH prerequisite. HERM-RUNNER owns runner liveness, bounded concurrency, teardown/signals, and diagnostic timeouts for that later root gate.
