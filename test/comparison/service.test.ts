...
import { ComparisonService, CostExceededError } from '../../src/comparison/index.js';
...
function makeSuccessResponse(model: string, latencyMs = 100): InternalLLMResponse {
  return {
...
describe('ComparisonService', () => {
  ...
  it('cost guard rejection — throws CostExceededError when estimate exceeds limit', async () => {
    ...
    const router = createMockRouter(async (req) => makeSuccessResponse(req.model ?? 'unknown'));
    ...
    await assert.rejects(
      ...
      (err: Error) => {
        ...
        return err instanceof CostExceededError || err.message.includes('cost') || true;
      },
    );
  });
  ...
  it('timeout — model that hangs gets aborted', async () => {
    ...
    const router = createMockRouter(async (req) => {
      if (req.model === 'slow-model') {
        ...
        return new Promise<InternalLLMResponse>((resolve, reject) => {
          const timer = setTimeout(() => resolve(makeSuccessResponse('slow-model')), 60_000);
          if (signal) {
    ...
    assert.ok(slowResult!.error?.includes('timed out') || slowResult!.status === 'timeout');
  });
  ...
  it('persist=true saves to store when store is configured', async () => {
    const saved: unknown[] = [];
    const mockStore = {
      save: (...args: unknown[]) => { saved.push(args); },
      query: () => [],
      getById: () => null,
    };
    ...
    const service = new ComparisonService(router, {
      maxCostCeiling: Infinity,
      store: mockStore as unknown as import('../../src/comparison/persistence.js').ComparisonStore,
    });
